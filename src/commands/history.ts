 ...
 */
async function executeHistory(options: HistoryOptions): Promise<void> {
  // Handle --clear
  if (options.clear) {
  ...
  if (options.json) {
    ...
    console.log(
      chalk.cyan(`Agent History${typeFilter} (${entries.length} of ${totalCount})\n`)
    );
...
export function registerHistoryCommand(program: Command): void {
  program
    .command("history")
    .description("View your agent's activity history (posts, comments, registrations)")
